...
testAllFeatures = assess "allFeatures" 2 $ do
                ...
                check "allFeatures is correct" $ quiet (allFeatures `shouldBe` [0..783])
...
testReqStyle = assess "required code style" 0 $ do
    ...
    builtsin <- check "that you do not use built-in versions of project functions" $ shouldNotBeCalled "lookup"
